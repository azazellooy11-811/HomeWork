...
object Utils {
    ...
    fun getTempColor(context: Context, temp: Double): Int {
        var color = 0;
        when (temp) {
            in Double.NEGATIVE_INFINITY ..TEMP_VERY_COLD -> color = context.getColor(
                R.color.colorVeryCold
            )
            in TEMP_VERY_COLD..TEMP_NORMAL_COLD -> color = context.getColor(
                R.color.colorNormalCold
            )
            in TEMP_NORMAL_COLD..TEMP_LOW_WARM -> color = context.getColor(
                R.color.colorLowCold
            )
            in TEMP_LOW_WARM..TEMP_NORMAL_WARM -> color = context.getColor(
                R.color.colorLowWarm
            )
            in TEMP_NORMAL_WARM..TEMP_VERY_WARM -> color = context.getColor(
                R.color.colorNormalWarm
            )
            in TEMP_VERY_WARM.. Double.POSITIVE_INFINITY -> color = context.getColor(
                R.color.colorVeryWarm
            )
        }
        return color
    }
